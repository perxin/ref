package com.ref.constant;

/**
 * Created by perxin on 2017/3/31 14:46.
 *
 */
public class PathConstants {

    ////////////////////////////////////////////////////////////

    public static final String ROUTE_MAIN = "S";
    public static final String ROUTE_MAIN_INDEX = "/home";
    public static final String ROUTE_MAIN_SIGN = "/sign";

    ////////////////////////////////////////////////////////////

    public static final String ROUTE_USER = "user";
    public static final String ROUTE_USER_SIGN_UP = "/sign_up";
    public static final String ROUTE_USER_SIGN_IN = "/sign_in";
    public static final String ROUTE_USER_SIGNOUT = "/sign_out";

    ////////////////////////////////////////////////////////////

    public static final String ROUTE_NIC_EDIT = "/nic";
    public static final String ROUTE_EDIT = "/edit";
    public static final String ROUTE_NOTE_ADD = "/note/add";
    public static final String ROUTE_NOTE_MODIFY = "/note/modify";
    public static final String ROUTE_NOTE_GET_PAGE = "/note/page";
    public static final String ROUTE_NOTE_GET_PAGE_HOT = "/note/page_hot";
    public static final String ROUTE_NOTE_GET_PAGE_NEW = "/note/page_new";
    public static final String ROUTE_NOTE_GET_NOTE_DETAIL = "/note";
    public static final String ROUTE_NOTE_COMMENT_ADD = "/comment";
    public static final String ROUTE_NOTE_PAGE = "note/notePage";

    ////////////////////////////////////////////////////////////

    public static final String ROUTE_FILE = "file";
    public static final String ROUTE_FILE_UPLOAD = "/upload";
    public static final String ROUTE_FILE_DOWNLOAD = "/download";
    public static final String ROUTE_FILE_PAGE = "/file";
}
